...
int main(int argc, char** argv) 
{
    ...
    { // start kokkos scope
        ...
        //auto old_start_from_3/28/22 = std::chrono::high_resolution_clock::now(); /* beginning timer */ 
       ...
       /* if dim is small, CHECK SOLUTIONS, using dim=5 */ 	
        Kokkos::deep_copy(A, h_A);
        Kokkos::deep_copy(B, h_B);  
		    Kokkos::deep_copy(tmp, h_tmp); 	
		 
		 //start kernel with nested parallelism 
        ...
        std::cout << "\nCells Still Alive: " << stillAlive(h_A, dim); 
	      ...
	      // TO DO: Add file output handling from serial_gol.cpp
		    /*for now, just uncomment these two lines below which will write results to csv*/ 
...
// no longer using this from serial version b/c it breaks on Cuda GPUs 
